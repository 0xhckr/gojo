...
func TestFileModeRendering(t *testing.T) {
	...
	out := renderDiffPanel(80, 24, "main.go", 0, false, false, 0, "", false, rows, digits, nil, "", 0, -1, -1, -1, nil, splitView{}, true, head, nil, -1)
	if len(out) != 24 {
...
func TestFileModeCursorBar(t *testing.T) {
	...
	out := renderDiffPanel(80, 24, "f.go", 0, false, false, 0, "", false, rows, digits, nil, "", 0, cursorBodyRow, -1, -1, nil, splitView{}, true, head, nil, -1)
	if len(out) != 24 {
		t.Fatalf("panel lines = %d, want 24", len(out))
	}
...
func TestFileModeStickyBlameHead(t *testing.T) {
	...
	out := renderDiffPanel(80, 24, "f.go", 0, false, false, 0, "", false, rows, digits, nil, "", termScrollY, cursorBodyRow, -1, -1, nil, splitView{}, true, head, nil, -1)
	if len(out) != 24 {
		t.Fatalf("panel lines = %d, want 24", len(out))
	}
	view := ansi.Strip(strings.Join(out, "\n"))

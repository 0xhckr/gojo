...
func TestSplitPanelRendering(t *testing.T) {
	...
	out := renderDiffPanel(80, 24, "abcd", 0, false, false, 0, "", false, rows, digits, nil, "", 0, -1, nil, nil, splitView{})
	plain := stripANSI(out)
	...
	sv := splitView{active: true, marked: map[int]bool{}}
	out = renderDiffPanel(80, 24, "abcd", 0, false, false, 0, "", false, rows, digits, nil, "", 0, -1, nil, nil, sv)
	if len(out) != 24 {

...
func TestRenderDiff(t *testing.T) {
	...
	out := renderDiffPanel(80, 24, "abcd", 0, false, false, 0, "", false, rows, maxLineDigits(rows), nil, "", 0, -1, nil, nil, splitView{}, false, nil)
	if len(out) != 24 {
...
func TestDiffWrap(t *testing.T) {
	...
	out := renderDiffPanel(m.width, m.height, m.diffRev, 0, false, false, 0, m.diffDesc, true, rows, m.diffDigits, m.diffStatus, "", m.diffScrollY, m.diffCursorBodyRow(), m.diffChunkRows(), nil, splitView{}, false, nil)
	if len(out) != m.height {
...
func TestDiffCollapseRendering(t *testing.T) {
	rows := renderDiff(sampleDiff)
	collapsed := map[string]bool{"foo.go": true}
	out := renderDiffPanel(80, 24, "abcd", 0, false, false, 0, "", false, rows, maxLineDigits(rows), nil, "", 0, -1, nil, collapsed, splitView{}, false, nil)
	if len(out) != 24 {
